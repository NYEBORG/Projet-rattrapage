...
int lireHistorique(Commande **historique) {
    ...
    FILE *fichier = NULL;
    fichier = fopen("history.csv", "r");
    while (!feof(fichier)) {
        size++;

        *historique = realloc(*historique, size * sizeof(Commande));
        fscanf(fichier, "%[^;];%[^;];%[^;];%[^;];%s", (*historique)[size - 1].nom,
...
void printHistorique(Commande *commande, int size) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        printDate(&(commande[i].debut), stringDebut);
        printDate(&(commande[i].fin), stringFin);
        printf("%s;%s;%s;%s;%s", commande[i].nom, commande[i].prenom, commande[i].immatriculation, stringDebut, stringFin);
    }
